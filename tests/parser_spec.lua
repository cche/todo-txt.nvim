...
describe("parser edge cases", function()
  it("extracts priority only when it is at start", function()
    ...
    assert.equals("A", parser.extract_priority("x (A) 2020-01-02 done"))
  ...
  it("extracts contexts and projects allowing - and _", function()
    ...
    assert.is_true(ctx["home-office"])
    assert.is_true(ctx["ctx2"])
    assert.is_nil(ctx["missing"])
    assert.is_true(proj["proj_name-1"])
    assert.is_true(proj["Another"])
    assert.is_nil(proj["nope"])
  end)
end)
